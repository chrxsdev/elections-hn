...
import type { ApiPayload, ElectionData, VoteRecords } from '../types/election';
...
export const fetchVoteRecords = async (): Promise<VoteRecords> => {
  const actasEndpoint = 'https://resultadosgenerales2025-api.cne.hn/esc/v1/presentacion-resultados/actas-validas';

  const response = await axios.post<VoteRecords>(actasEndpoint, API_PAYLOAD, {
    headers: {
      accept: 'application/json, text/plain, */*',
      'content-type': 'application/json',
      authorization: 'Bearer null',
